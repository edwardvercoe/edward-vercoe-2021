...
void main() {
    ...
    newpos.yz += .7*progress1*(cnoise(vec4(noise2, time/5.)) + 0.2);


    // newpos.z += cnoise(vec4(position, time));
    // newpos.y += cnoise(vec4(position, time*.3));
    // newpos.x += cnoise(vec4(position, time*.5));

    
    
    vec4 mvPosition = modelViewMatrix * vec4( newpos, 1.);
    // particle size radius ( first number )
